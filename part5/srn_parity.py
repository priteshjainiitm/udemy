# -*- coding: utf-8 -*-
"""
Created on Mon Jun 12 17:57:16 2017
...
class SimpleRNN:
    ...
    def fit(self, X, Y, learning_rate=10e-1, mu=0.99, reg=1.0, activation=T.tanh,epochs=100, show_fig=False):
        ...
        def recurrence(x_t, h_t1):
            #h_t1 is previous state, x_t is current x
            #this returns h(t) and y(t)
        ...
        dparams = [theano.shared(p.get_value()*0) for p in self.params]#*0 to get same shape as p
        ...
        for i in xrange(epochs):
            ...
            cost = 0
            for j in xrange(N):
                c,p,rout = self.train_op(X[i], Y[j])
                cost += c
                if p[-1] == Y[j, -1]:
                    n_correct += 1
