# -*- coding: utf-8 -*-
"""
Created on Fri Jun  2 18:18:36 2017
...
class HiddenLayer(object):
    def __init__(self, M1, M2, an_id):
        ...
        self.M1 = M1
        self.M2 = M2
        W = np.random.randn(M1, M1)/np.sqrt(M1+M1)
        b = np.zeros(M2)
        self.W = theano.shared(W, 'W_%s' % self.id)
        self.b = theano.shared(b, 'b_%s' % self.id)
        self.params = [self.W, self.b]
        #dropout not included in hidden layer but outside
        
    def forward(self, X):
        return T.nn.relu(X.dot(self.W) + self.b)
...
class ANN(object):
    ...
    def fit(self, X, Y, learning_rate = 10e-7, mu = 0.99, decay = 0.999, epochs = 300, batch_sz = 100, show_fig = False):
        ...
        #calling it forward_train instead of train bcoz dropout acts different during training and prediction
        
        #this is the cost for training
        ...
        cost_predict = -T.mean(T.log(pY_predict[T.arange(thX.shape[0]), thY]))
